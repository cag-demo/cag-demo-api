...
echo running  docker run -d -e "DOODLE_ENV=${DOODLE_ENV}" --link dockerapplicationserver_elk_1:elk --name cag-demo-api-${INSTANCE} -p ${PORT_PREFIX}${PORT}:${PORT} -p ${PORT_PREFIX}${M_PORT}:${M_PORT} cag-demo-api:$VERSION
docker run -d -e "DOODLE_ENV=${DOODLE_ENV}" --link dockerapplicationserver_elk_1:elk --name cag-demo-api-${INSTANCE} -p ${PORT_PREFIX}${PORT}:${PORT} -p ${PORT_PREFIX}${M_PORT}:${M_PORT} cag-demo-api:$VERSION
